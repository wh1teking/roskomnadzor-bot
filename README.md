...
```
Данная ошибка происходит из-за того, что `prismarine-viewer` использует тот же самый порт при запуске двух скриптов сразу, тем самым конфликтуя и вызывая данную ошибку.
Если вам очень сильно надо два и больше скрипта, запущенных одновременно, то сделайте копию и измените порт в функции createViewer. (line 148)
